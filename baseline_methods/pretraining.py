...
class PretrainingBaseline:
    ...
    """
    def __init__(self, loss_function, l2_lambda=0.01):
        self.model_rln = None
        self.model_tln = None
        self.loss_function = loss_function
        self.l2_lambda = tf.constant(l2_lambda)
    ...
    def _compute_loss(self, x, y):
        ...
        return self.loss_function(y, self.model_tln(self.model_rln(x))) + self.l2_lambda * l2

    def pre_train(self, x_pre_train, y_pre_train, optimizer, batch_size=1):
        data = tf.data.Dataset.from_tensor_slices((x_pre_train, y_pre_train)).batch(batch_size)
        full_loss = 0
        ...
        for x, y in data:
            ...
            full_loss += loss
        return full_loss / len(x_pre_train)
