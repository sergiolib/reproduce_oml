...
argument_parser.add_argument("--n_functions", type=int, default=400,
...
train_log_dir = 'logs/isw_oracle/' + current_time + '/train'
makedirs(train_log_dir, exist_ok=True)

n_ids = 10

# Main pre training loop
rln, tln = mrcl_isw(one_hot_depth=n_ids)  # Create and initialize models
...
x_traj, y_traj, x_rand, y_rand = gen_sine_data(tasks=tasks, n_functions=args.n_functions,
                                               sample_length=args.sample_length,
                                               repetitions=args.repetitions, n_ids=n_ids)
...
# Oracle data so shuffle everything
x_traj = np.reshape(x_traj, (-1, n_ids + 1))
y_traj = np.reshape(y_traj, (-1,))

traj_inds = np.random.permutation(len(x_traj))
rand_inds = np.random.permutation(len(x_rand))

x_traj = x_traj[traj_inds]
y_traj = y_traj[traj_inds]

x_rand = x_rand[rand_inds]
y_rand = y_rand[rand_inds]

x_traj = np.reshape(x_traj, (-1, args.sample_length, n_ids + 1))
y_traj = np.reshape(y_traj, (-1, args.sample_length))
